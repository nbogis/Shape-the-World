...
unsigned long Led,InOut;
void Delay(void){ // delay of 0.05sec (160000/2=80000, 0.1/2=0.05)
	unsigned long volatile time;
  //time = 160000; // 0.1sec
	time = 80000; 	// 0.05sec (10Hz) 
  while(time){
   time--;
  }
}
...
int main(void){  unsigned long i,last,now;
  ...
  InOut = GPIO_PORTF_DATA_R&0x13;
	while(1){
		Led = GPIO_PORTF_DATA_R;   // read previous
		if ((Led&0x11)==0x11){		// both switches are not pressed
			Led = Led & ~0x02;		// make led low
		}
		else if (((Led&0x11)==0x10) || ((Led&0x11)==0x01) || (Led&0x11)==0x00){
			Led = Led^0x02;            // toggle red LED
		}
		GPIO_PORTF_DATA_R = Led;   // output 
		if ((GPIO_PORTF_DATA_R&0x13) != InOut){
			if(i<50){
